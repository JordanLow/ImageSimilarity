"""Step 2 of 4 (LightGlue variant) — C1 ablation for NCR-Match Stage 2.

Drop-in replacement for geometry_filter.py that uses kornia LightGlue + DISK
instead of ASpanFormer. The output manifest schema and NPZ sidecar format are
identical to geometry_filter.py, so vggt_signals.py (Step 3) and pose_scoring.py
(Step 4) run completely unchanged.

The matching pipeline mirrors geometry_filter.py step-by-step:
  1. Load both images and resize to long_dim (same resize logic).
  2. Extract DISK keypoints from each image.
  3. Match with LightGlue; optionally filter by match confidence.
  4. Apply fundamental-matrix RANSAC (same cv2.FM_RANSAC parameters as
     geometry_filter.py) to compute filtered keypoints and the sidecar payload.
  5. Write aspan_all_manifest (all pairs) and vggt_candidates_manifest (passed pairs).
  6. Save NPZ sidecars for passed pairs — same arrays, same field names.

Requirements:
    pip install kornia>=0.7.0
    (kornia is already installed in the standard Colab GPU runtime)

Usage (Colab):
    python geometry_filter_lightglue.py \\
        --input-manifest /path/to/retrieval_manifest.jsonl \\
        --output-dir    /path/to/lightglue_output/ \\
        --breakpoint-value 50 \\
        [--long_dim 1024] \\
        [--n-keypoints 2048] \\
        [--resume]
...
# ── Shared I/O helpers (identical to geometry_filter.py) ──────────────────────
...
def pad_to_stride(img: np.ndarray, stride: int = 16) -> np.ndarray:
    """Pad image height and width to the next multiple of stride (bottom/right padding)."""
    h, w = img.shape[:2]
...
# ── LightGlue model loading ───────────────────────────────────────────────────

def load_lightglue(device: torch.device):
    """Load DISK feature extractor + LightGlue matcher from kornia.

    Requires kornia >= 0.7.0.  In a standard Colab GPU runtime:
    ...
    try:
        ...
        raise ImportError(
            "kornia >= 0.7.0 is required: pip install -U kornia"
        )
    extractor = DISK.from_pretrained("depth").to(device).eval()
    matcher   = LightGlue("disk").to(device).eval()
    return extractor, matcher


# ── LightGlue pair matching ───────────────────────────────────────────────────

def run_lightglue_pair(
    extractor,
    matcher,
    source_path: str,
    target_path: str,
    long_dim: int,
    device: torch.device,
    n_keypoints: int = 2048,
) -> dict[str, Any]:
    """Run LightGlue on one image pair.
    ...
    # DISK expects float RGB (B, 3, H, W) in [0, 1]
    def to_tensor(bgr: np.ndarray) -> torch.Tensor:
        rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
        return torch.from_numpy(rgb).permute(2, 0, 1)[None].float().to(device) / 255.0

    # DISK's U-Net backbone requires H and W divisible by 16
    img0_t = to_tensor(pad_to_stride(img0_resized))
    img1_t = to_tensor(pad_to_stride(img1_resized))

    with torch.inference_mode():
        # DISK.forward() returns List[DISKFeatures]; index [0] for the single image in batch
        disk0 = extractor(img0_t, n=n_keypoints)[0]
        disk1 = extractor(img1_t, n=n_keypoints)[0]
        feats0 = {
            "keypoints":   disk0.keypoints.unsqueeze(0),    # (1, N, 2)
            "descriptors": disk0.descriptors.unsqueeze(0),  # (1, N, D)
            "image":       img0_t,
        }
        feats1 = {
            "keypoints":   disk1.keypoints.unsqueeze(0),
            "descriptors": disk1.descriptors.unsqueeze(0),
            "image":       img1_t,
        }
        result = matcher({"image0": feats0, "image1": feats1})

    # kornia LightGlue returns "matches0": (B, N0) where each value is the
    # index of the matched keypoint in img1, or -1 for unmatched keypoints.
    kpts0_all = feats0["keypoints"][0]   # (K0, 2) in resized-image pixel coords
    kpts1_all = feats1["keypoints"][0]   # (K1, 2)
    m0 = result["matches0"][0]            # (K0,) — index into kpts1_all, -1 = unmatched
    valid = m0 >= 0
    if valid.any():
        idx0 = torch.where(valid)[0]
        idx1 = m0[valid]
        raw0 = kpts0_all[idx0].cpu().numpy().astype(np.float32)
        raw1 = kpts1_all[idx1].cpu().numpy().astype(np.float32)
    else:
    ...
    if raw_count >= 8:
        ...
        ransac_error = f"not enough LightGlue matches for RANSAC: {raw_count}"
...
def process(args: argparse.Namespace) -> None:
    ...
    print(f"Loading LightGlue + DISK (n_keypoints={args.n_keypoints}) ...")
    extractor, matcher = load_lightglue(device)
    print("Models loaded.")
    ...
    for _line_no, candidate in read_jsonl(args.input_manifest):
        ...
        try:
            ...
            match = run_lightglue_pair(
                extractor, matcher,
                source_path, target_path,
                args.long_dim, device, args.n_keypoints,
            )
            ...
            audit_row = dict(base)
            audit_row.update({
                "matcher":          "lightglue+disk",
                "n_keypoints_cfg":  args.n_keypoints,
...
def parse_args(argv=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="LightGlue geometry filter — C1 ablation replacement for geometry_filter.py"
    )
    ...
    p.add_argument("--long_dim",  type=int, default=1024,  help="Resize long edge to this many pixels")
    p.add_argument("--n-keypoints", type=int, default=2048, help="Max DISK keypoints per image")
    p.add_argument("--device", default="auto", help="auto, cuda, or cpu")
    p.add_argument("--all-manifest-name",    default="lightglue_all_manifest.jsonl")
    ...
    p.add_argument("--sidecar-dir", default="lightglue_sidecars")
