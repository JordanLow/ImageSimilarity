...
## Abstract v3 — 当前定稿（2026-07-22 与德乾会后修订，≈261 词，全文无破折号）
...
We instantiate this on the North China Railway Archive (華北交通アーカイブ) and the magazine Hokushi (北支, 1939 to 1942), and release NCR-Match, an expert-adjudicated dataset spanning 42,773 archival photographs, 2,856 printed reproductions, and more than 1,200 candidate pairs labeled same exposure, same scene, or unrelated. We propose NEG-Net, a relational model tailored to this structure: it scores candidate pairs jointly over the corpus rather than independently, and trains on only a few hundred expert adjudications. NEG-Net agrees with expert adjudication on [XX]% of held-out pairs, [+X] points over an otherwise identical model that scores pairs independently, and its calibrated three-way output bounds the false-rejection rate at [α] while reducing pairs requiring expert review by [XX]%. Zero-shot transfer to the Sha Fei archive and Jinchaji Pictorial yields [XX].

**TL;DR（≤250 字符，二选一）**
- A（232 字符）：We recast matching printed reproductions to archival photograph negatives as nested partition estimation, release NCR-Match, an expert-adjudicated dataset, and propose NEG-Net, a relational model with calibrated three-way decisions.
- B（237 字符）：NCR-Match: an expert-adjudicated dataset for matching printed reproductions to archival photo negatives, plus NEG-Net, a relational model trained on few labels whose calibrated accept/review/reject output bounds the false-rejection rate.

**v3 落实的会议意见（260722）**：删除全部具体模型名（管线细节留给正文）；模型定名 NEG-Net；NCR-Match 改称 dataset（将发布于 Hugging Face）并一句话给出规模（42,773 / 2,856 / 1,200+ 标注对）；保留并保持人文/civic 动机段；全文不用破折号。占位符：[XX]% 一致率、[+X] 相对独立打分模型的提升（现有数据为跨批 +2 F1）、[α] 误拒率上界、复核量减少 [XX]%、JOCCH 迁移 [XX]。
