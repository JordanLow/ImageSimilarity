...
- **Two resolutions, pick one before the dataset-release claim is final**:
  ...
  - **Presentation (resolves the asymmetry concern)**: not "a θ_S result" but **stratified error analysis on the hardest negative class** — one table, both corpora: per decision layer (B4 / mp0 / mp3), false-positive rate on S\N negatives vs unrelated negatives. Quantifies exactly where false positives concentrate and lets the taxonomy earn its place; θ_S's own S\N-vs-U separation goes in as one exploratory, clearly-caveated sentence or is dropped (Jordan's call).
  - **Why the small stratum is valid for the FP table (state in one sentence in §6)**: false positives are by construction high-evidence negatives, so every system's FPs fall almost entirely inside the labeled top-similarity stratum — for "where do FPs fall," the stratum is near-complete coverage, not a biased sample; numbers reported stratum-conditional. No retraining on the new labels before the freeze under any scenario; full-audit-then-retrain is the stated future work.
