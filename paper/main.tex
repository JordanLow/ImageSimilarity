...
surfaces. Two survival analyses address this at different scopes. First,
\emph{within the labeled candidate sets}: of the 325 confirmed positives
on Shard~1, the inlier gate removes 9 and the pose gate a further 3
(96.3\% survive); of the 252 on Shard~2, the inlier gate removes none and
the pose gate 4 (98.4\%); jointly, 561 of 577 confirmed positives
(97.2\%) survive the two decision gates. These figures cannot speak to
the retrieval and correspondence-floor stages, because labels exist only
for pairs that already passed them. Second, therefore, the full-corpus
audit: Table~\ref{tab:survival} pushes all 2{,}636 verified pairs through
every stage at frozen thresholds and reports the fraction still alive
after each. That table is the paper's only end-to-end claim, and its
final row answers the historian's operative question: \emph{of the
reproductions known to exist, how many does the system find?}
