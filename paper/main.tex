...
recall, conditional on the candidate set. We release NCR-Match, an
expert-adjudicated benchmark of candidate photograph--print pairs with
exposure-identity versus same-scene labels --- 577 confirmed same-exposure
pairs and 702 same-scene negatives at submission time, growing as further
shards are labeled \todo{final counts after the Shard 3--6 campaigns} ---
drawn from the North China Railway Archive, a CC~BY~4.0 occupation-era
collection, and evaluate cross-system transfer to an opposing propaganda
...
\item \textbf{Method.} Inferred relative camera pose as a provenance signal,
  ...
  inputs \todo{remaining Table~\ref{tab:verification} rows: B8
  (essential-matrix pose), B9 (MASt3R, run W6), B11--B14 (alignment and
  matcher perturbations, runs W7/W8), plus all Shard-3 columns}.
\item \textbf{Benchmark.} NCR-Match: an expert-adjudicated benchmark of
  candidate photograph--print pairs linking the North China Railway Archive
  ...
  Every pair carries an exposure-identity label (same exposure / same-scene
  negative / Unsure) assigned under a written codebook: 1{,}279 labeled
  pairs at submission time (577 positive, 702 negative), growing as further
  shards are labeled \todo{Shard 3--6 counts}. Archive images and
...
\item \textbf{Historian-in-the-loop evaluation and findings.} An annotation
  ...
  first-class outcome}, a measured workflow-impact analysis (review-queue
  reduction and expert adjudication throughput \todo{adjudication timing
  from the Shard-3 campaign}),
...
Archive photographs (1939--45) & $\sim$39{,}000 \\
\emph{North China} issues (1939--43) & \todo{n} \\
Working corpus (YOLOv11 crops) & 42{,}773 \\
\midrule
\multicolumn{2}{@{}l}{\emph{Expert-labeled benchmark (NCR-Match)}}\\
Labeled candidate pairs & 1{,}279 \todo{+S3--6} \\
\quad same-exposure / same-scene neg. & 577\,/\,702 \\
\midrule
\multicolumn{2}{@{}l}{\emph{Labeled pairs (pos.\,/\,neg.\,/\,Unsure)}}\\
Shard 1 (development) & 325\,/\,316\,/\,2 \\
Shard 2 (validation, frozen) & 252\,/\,386\,/\,0 \\
Shard 3 (test, evaluated once) & \todo{n\,/\,n\,/\,n} \\
Inter-annotator agreement ($\kappa$) & \todo{$\kappa$} \\
\midrule
\multicolumn{2}{@{}l}{\emph{Transfer domain (Jinchaji\,/\,Sha Fei)}}\\
Photographs\,/\,magazine pages & 2{,}146\,/\,1{,}122 \\
Crops\,/\,annotated pairs & 4{,}390\,/\,230 \\
\bottomrule
\end{tabular}
\caption{NCR-Match at a glance. Labels exist for candidate pairs
that survive retrieval and the correspondence floor; all verification metrics
are therefore conditional on the candidate set (see Experiments), and the
survival analysis is likewise scoped to the expert-labeled pairs.}
\label{tab:dataset}
\end{table}
...
\textbf{Candidate generation and expert labeling.} Candidate photo--print
pairs are proposed by embedding retrieval (top-10 per source photograph)
followed by the correspondence floor, which reduces roughly 70{,}000
candidate pairs per shard to $\sim$640 requiring expert eyes. Every
surviving candidate is then labeled by hand as \emph{same exposure},
\emph{same-scene negative}, or \emph{Unsure}, under a written codebook that
fixes the genuinely contested cases (adjacent contact-sheet frames; crops of
a same-scene negative). Each labeled pair carries magazine issue, date, and
page metadata. The label composition is itself evidence for the task:
49\%\,/\,61\% of retrieved-and-geometrically-plausible candidates on the two
labeled shards are \emph{not} the same exposure --- look-alikes dominate the
candidate space, which is precisely why a dedicated exposure-identity
decision is needed. Confirmed same-scene negatives are released alongside
the positives --- they are the benchmark's hard-negative class.
...
the 577 confirmed positives (extend as further shards are labeled), expert
taxonomy tagging}; per-type pipeline
...
documents collection, the labeling codebook and adjudication protocol, and
...
with per-stage survivor counts from the labeled-shard survival analysis}
\vspace{2em}}}
...
count, the log narrative says raw), confirm the retrieval top-$K$ (the
method and log say top-10; one analysis artifact says top-15), and verify
that the resolved definitions
regenerate exactly the 641/638 labeled candidate sets of Shards~1--2 that
produced rows B2/B3; row~B1 sweeps the floor over the full observed
correspondence range on the development shard (best-F1 floor 2{,}375 raw
correspondences, applied frozen to Shard~2), CPU-only from
\texttt{aspan\_all\_manifest.jsonl}}. The floor is a compute
...
Every metric below names its denominator. \emph{Retrieval-stage} comparisons
are scoped to the 577 expert-confirmed positive pairs and carry a printed
caveat: those pairs were \emph{discovered through} this pipeline's own
retrieval, so any retrieval statistic over them is biased in our encoder's
favor; we use them only to compare encoders against one another, never as an
archive-level recall claim. Full-archive recall is not reported by design:
measuring it would require already knowing every true pair in a
$\sim$39{,}000-photograph archive, which is the problem the system exists to
address. \emph{Verification} precision, recall, and F1 are \emph{conditional
on the candidate set}: ground-truth labels exist for the candidate pairs
that survive retrieval and the correspondence floor, and all metrics in
Table~\ref{tab:verification} are computed over these expert-adjudicated
pairs. We never quote a verification number as end-to-end performance. The
paper's strongest end-to-end statement is the survival analysis of
Table~\ref{tab:survival}, whose denominator is the expert-confirmed
positives of the labeled shards.
...
off-the-shelf descriptors under identical preprocessing, in the spirit of
the distractor-corpus protocol of the Image Similarity Challenge
\cite{douze2021disc}, scoped to the 577 expert-confirmed positives with the
discovery-bias caveat of the Protocol subsection printed alongside. A GeM
descriptor \cite{radenovic2019gem} represents the classical
landmark/instance-retrieval family (GeM/DELG) alongside the copy-detection
and foundation-model encoders. Two rows isolate our design choices:
DINOv3-alone with the SSL head (does the three-backbone fusion beat its
best member?) and fusion without SSL (does the SimCLR-style head
\cite{chen2020simclr} matter?). Because an alternative encoder may surface
candidate pairs our own retrieval never proposed, every \emph{new}
candidate that survives the frozen decision gates is forwarded to the
domain expert for adjudication and added to the released labels with its
provenance recorded \todo{counts of newly surfaced candidates per encoder
and their adjudicated outcomes --- this both audits our encoder's blind
spots and grows the benchmark}.
...
\midrule
A1 & SSCD & \multicolumn{5}{c}{\todo{W4}} \\
A2 & DINOv2 global & \multicolumn{5}{c}{\todo{W4}} \\
A3 & DINOv3 global & \multicolumn{5}{c}{\todo{W4}} \\
A4 & CLIP / SigLIP~2 & \multicolumn{5}{c}{\todo{W4}} \\
A5 & GeM & \multicolumn{5}{c}{\todo{W4}} \\
A6 & ViT ensemble & \multicolumn{5}{c}{\todo{W4}} \\
...
\bottomrule
\end{tabular}
\caption{Retrieval-stage comparison over the 577 expert-confirmed positive
pairs with the 42{,}773-crop corpus as distractors (discovery-bias caveat
in Protocol). Encoders: SSCD \cite{pizzi2022sscd}; DINOv2
\cite{oquab2024dinov2}; DINOv3 \cite{simeoni2025dinov3}; CLIP
\cite{radford2021clip} / SigLIP~2 \cite{tschannen2025siglip2}; GeM
\cite{radenovic2019gem}; the ViT ensemble of \citet{du2024probing}.
\todo{All cells: W4 = featurize the corpus with each frozen encoder and
score Recall@$K$/mAP; W9 = retrain rows A8--A9. A6 is reimplemented from
the published description (confirm weights are publicly released before
claiming otherwise).}}
\label{tab:retrieval}
\end{table}
...
$8.6\times10^{-4}$ (Shard~2). After Holm correction across the Shard-2
tests of rows B1--B5, every difference except the rotation$+xy$ row (B4,
$p=0.50$) remains significant at $\alpha=0.05$; the learned-classifier
comparison (B10) awaits a matched-variant recomputation.
...
(rotation angle and $xy$-translation) and its low-weight terms (fov shift
and $z$-translation). The outcome matches the design: the low-weight gate
rejects nothing at the published threshold --- B5 is decision-identical
to the pose-free rule B2 --- while B4 alone is statistically
indistinguishable from the full system ($p=0.50$): rotation and lateral
translation carry essentially all of the pose signal. The per-component
distributions agree (Figure~\ref{fig:pose-components}): single-component
AUCs are 0.948\,/\,0.973 for rotation and 0.929\,/\,0.957 for
$xy$-translation, against 0.830--0.893 for $z$ and fov. Row~B6 gates on
the global feature-similarity signal that the pipeline records but
\emph{excludes} from its decision rule: a dev-tuned gate on it reaches
only F1 77.5\,/\,72.6 with PR-AUC 0.853\,/\,0.838 --- far below every
geometric row --- which is why it is recorded but not used.
\emph{Alternative-pose rows} B7--B9 answer the question a reader should
ask: the alignment stage already estimates a homography, and decomposing
it yields relative rotation and translation for free
...
as our gate was. The classical alternative is genuinely competitive ---
the strongest form of the objection --- but consistently weaker: at its
dev-tuned $4.5^{\circ}$ rotation threshold, B7 gives up 4.9 recall points
on Shard~1 and 2.3 F1 points on Shard~2 relative to the full system, and
its threshold-free PR-AUC trails the VGGT score by 4.2 and 3.1 points
(0.922\,/\,0.962 vs.\ 0.964\,/\,0.993), a gap that is stable under
$\pm 30\%$ focal-length misspecification. Row~B10 replaces the hand-set
thresholds with a logistic-regression classifier over the stored per-pair
signals, in the spirit of Doppelgangers \cite{cai2023doppelgangers},
trained on Shard~1 with a cross-validated threshold. The fitted rule
trades recall for precision (98.0\,/\,95.6 vs.\ the full system's
90.2\,/\,98.4 on Shard~2) \todo{recompute the paired McNemar test on this
exact LR variant --- the current test was run on a different feature
configuration and cannot be quoted for this row}. We nonetheless keep the
two-threshold rule as the deployed default: it is interpretable, and it favors recall --- in
provenance work a missed pair is a discovery permanently lost, while a
false positive is caught in adjudication --- but B10 shows clear headroom
for a learned decision layer. Rows B11--B14 perturb the geometry stage
itself (alignment off; filtered vs.\ raw alignment correspondences;
matcher swaps to LightGlue \cite{lindenberger2023lightglue} and RoMa
\cite{edstedt2024roma}).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/pose_signal_analysis.png}
\caption{Per-component pose-delta distributions for exposure-identical
pairs vs.\ same-scene negatives among correspondence-floor survivors.
Exposure-identical pairs concentrate near zero on every component.
Single-component AUCs --- rotation 0.948\,/\,0.973, $xy$-translation
0.929\,/\,0.957, $z$ 0.830\,/\,0.866, fov 0.891\,/\,0.893 (Shard~1\,/\,2)
--- support the weighting asymmetry of Eq.~\ref{eq:pose-score}.}
...
B1 & Correspondence floor (dev 2{,}375) & 88.4 / 89.2 / 88.8 & 87.8 / 91.7 / 89.7 & \todo{S3} & 0.962 & $8.1\times10^{-3}$ \\
B2 & + inlier-ratio gate (no pose) & 82.9 / 97.2 / 89.5 & 83.7 / 100 / 91.1 & \todo{S3} & 0.996 & $8.6\times10^{-4}$ \\
B3 & \textbf{+ weighted pose gate (full)} & \textbf{86.7 / 96.3 / 91.3} & \textbf{90.2 / 98.4 / 94.1} & \todo{S3} & 0.993 & --- \\
\midrule
B4 & Pose: rotation + $xy$ only & 86.2 / 96.3 / 91.0 & 89.5 / 98.4 / 93.8 & \todo{S3} & --- & 0.50 \\
B5 & Pose: fov + $z$ only & 82.9 / 97.2 / 89.5 & 83.7 / 100 / 91.1 & \todo{S3} & --- & $8.6\times10^{-4}$ \\
B6 & Global-similarity gate (dev 0.955) & 70.3 / 86.5 / 77.5 & 61.1 / 89.3 / 72.6 & \todo{S3} & 0.838 & --- \\
\midrule
B7 & Homography-decomp.\ pose (dev 4.5$^{\circ}$) & 86.6 / 91.4 / 88.9 & 86.9 / 97.2 / 91.8 & \todo{S3} & 0.962 & \todo{pairing} \\
B8 & Essential-matrix pose & \multicolumn{5}{l}{\todo{five-point + RANSAC on cached correspondences; CPU}} \\
B9 & MASt3R relative pose & \multicolumn{5}{l}{\todo{W6: same tune-on-S1 protocol}} \\
B10 & Learned classifier (LR, train S1) & --- & 98.0 / 95.6 / 96.8 & \todo{S3} & 0.993 & \todo{matched} \\
\midrule
B11 & Alignment off & \multicolumn{5}{l}{\todo{W8: VGGT on unaligned crops}} \\
B12 & Filtered alignment corresp. & \multicolumn{5}{l}{\todo{W8: raw vs.\ filtered correspondences}} \\
B13 & Matcher swap: LightGlue & \multicolumn{5}{l}{\todo{W7: re-tune on S1, frozen on S2/S3}} \\
...
on Shard~1 only and applied frozen elsewhere. McNemar $p$-values are
computed on paired decisions (exact binomial for small discordant counts,
continuity-corrected $\chi^2$ otherwise); Holm correction across the table
...
Top left: inlier-ratio sweep (0.65 marked). Top right: PR/ROC over the
continuous pose score among inlier-gate survivors (PR-AUC
0.971\,/\,0.966; $\star$ = 2.13). Bottom left: 2-D F1 surface over both
thresholds on the development shard ($\star$ = published operating point;
Shard-2 inset) --- a broad plateau; the published point deliberately sits
on its recall-leaning side rather than at the F1 maximum. Bottom right:
within-labeled-set Recall@$K$. \todo{replot the 2-D surface on Shard~3
after W2/W3}}
\label{fig:sensitivity}
\end{figure}

\subsection{Survival of Confirmed Pairs Through the Decision Gates}

Verification metrics above score decisions pair by pair; the survival
analysis of Table~\ref{tab:survival} asks the cumulative question instead:
of the pairs experts confirmed to be the same exposure, how many emerge
from the decision gates intact? Jointly, 561 of 577 confirmed positives
(97.2\%) survive. We state the scope plainly: this analysis begins at the
decision gates, because labels exist only for candidates that already
passed retrieval and the correspondence floor. A full-archive recall ---
``of every reproduction that exists in the archive, how many does the
system find?'' --- is not reported by design and, we argue, cannot be
reported honestly by anyone: answering it presupposes a complete
ground-truth pairing of a $\sim$39{,}000-photograph archive, which is the
very problem the system exists to address (see the transfer experiment
for the one setting where an independent, pre-pipeline annotation
exists).
...
\begin{tabular}{@{}lccc@{}}
\toprule
Stage & Shard 1 & Shard 2 & Joint \\
\midrule
Expert-confirmed positives & 325 & 252 & 577 \\
survive inlier gate ($\geq 0.65$) & 316 & 252 & 568 \\
survive pose gate ($\leq 2.13$) & 313 & 248 & 561 \\
\midrule
Survival rate & 96.3\% & 98.4\% & \textbf{97.2\%} \\
\bottomrule
\end{tabular}
\caption{Survival of expert-confirmed same-exposure pairs through the two
frozen decision gates. Scope: labeled candidate pairs (labels exist only
downstream of retrieval and the correspondence floor). \todo{add the
Shard-3 column after the labeling campaign}.}
\label{tab:survival}
\end{table}
...
photographs, 1{,}122 magazine pages, 4{,}390 crops): Communist base-area
propaganda, produced under different printing technology and degradation
than the Japanese-run \emph{North China} corpus, and studied previously by
\citet{du2024probing}, whose system reached 77.8\% top-15 retrieval on
this collection. This domain also carries the one annotation in either
collection that predates any retrieval model: pairs identified
\emph{entirely by hand} by three trained annotators who memorized batches
of source photographs and searched the printed pages by eye
\cite{du2024probing}, of which 230 were used as that paper's evaluation
set. Because those labels owe nothing to any pipeline, retrieval recall
measured against them carries no discovery bias --- with the converse
caveat, which we also print, that manual search cannot guarantee
exhaustiveness, so they are a floor, not a census. We report Recall@10/15
against this annotation \todo{W10: frozen-pipeline Jinchaji run; confirm
the annotation file is recovered and its provenance documented} and
...
NCR-Match consists of expert-labeled candidate pairs, and the reliability
of those labels is itself an evaluated quantity. For the shard labels used
in Table~\ref{tab:verification}, a written one-page codebook fixes the
...
\paragraph{Workflow impact.}
We quantify the system's contribution to the historian's workflow as
reduction of the review queue, with named denominators. A manual baseline
does not exist at this scale: exhaustively comparing $\sim$39{,}000
archive photographs against the 4{,}558 magazine crops is on the order of
$1.8\times10^{8}$ pairwise judgements --- at ten seconds per judgement,
decades of person-time. The funnel reduces the roughly 70{,}000 top-10
candidate pairs of each labeled shard to 641 and 638 pairs requiring
expert eyes, a two-orders-of-magnitude reduction, and the verification
stage then concentrates truth within that queue: the class prior of the
candidate set is 50.7\%\,/\,39.5\% positive (Shards~1\,/\,2), whereas
86.7\%\,/\,90.2\% of what the system finally asserts is a confirmed
positive. Expert adjudication throughput on this queue is
\todo{median seconds per pair, logged during the Shard-3 labeling
campaign}, so a shard's verified output costs hours of expert time
rather than months. A controlled within-subjects user study with
independent historians --- tool vs.\ plain retrieval list, counterbalanced
--- is planned as future work; we do not claim measured time savings
beyond the queue-reduction arithmetic above.
...
These findings cost the historian hours, not months: each begins from a
review queue of a few hundred system-verified pairs rather than an
archive of tens of thousands of photographs (see the workflow-impact
analysis in the Human Evaluation subsection \todo{insert the measured
adjudication throughput once logged during the Shard-3 campaign}).
...
is reported as the survival analysis of Table~\ref{tab:survival},
scoped explicitly to the expert-labeled pairs, rather than implied to
be complete.
...
records collection provenance, the labeling codebook and adjudication
protocol, and --- centrally --- the archive's own selection bias: the North
...
the candidate set; the survival analysis is scoped to expert-labeled
pairs, and full-archive recall is unmeasurable by design --- the
strongest coverage statement we can make is the 97.2\% gate-survival
rate plus the discovery-bias disclosure.
...
auditable decision layer. We release NCR-Match --- expert-adjudicated
candidate pairs with exposure-identity and same-scene-negative labels
(1{,}279 at submission, growing shard by shard), with datasheet and
code --- deposited, not promised. With
